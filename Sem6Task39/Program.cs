// Напишите программу, которая перевернёт одномерный массив (последний элемент будет на
//первом месте, а первый - на последнем и т.д.)

int[] arr = GenArray(20,1,100);     //генерируем массива
Console.WriteLine("Исходны массив"); //
Print1DArr(arr);                     // печатаем массив, который получили

int[] copyArr = SwapNewArray(arr);
Console.WriteLine("Новый массив перевернутый SwapNewArray методом");
Print1DArr(copyArr);
Console.WriteLine("Исходны массив");
Print1DArr(arr);

int[] copyArrTwo = SwapArray(arr);
Console.WriteLine("Исходны массив");
Print1DArr(arr);
Console.WriteLine("Исходный массив перевернутый SwapArray методом");
Print1DArr(copyArrTwo);

// Метод, генерирующий массив
int[] GenArray(int len, int minValue, int maxValue)
{
    int[] arr = new int[len];
    for (int i = 0; i < arr.Length; i++)
    {
        arr[i] = new Random().Next(minValue, maxValue + 1);
    }
    return arr;
}

//Метод, выводящий на печать массив
void Print1DArr(int[] arr)
{
    Console.Write("[");
    for (int i = 0; i < arr.Length - 1; i++)
    {
        Console.Write(arr[i] + ", ");
    }
    Console.WriteLine(arr[arr.Length - 1] + "]");
}
// простой разворот
int[] SwapArray(int[] arr) 
{
    int bufElement = 0;  // создаем буферный элемент
    for(int i =0;i<arr.Length/2;i++) // проходим по массиву до половины
    {
        bufElement = arr[arr.Length-1-i]; // последний элемент из массива кладем в буферный элемент
        arr[arr.Length-1-i] = arr[i];  //
        arr[i] = bufElement;  // возвращаем вначало элемент из буфера
    }
    return arr;
}

// создает новый массив
int[] SwapNewArray(int[] arr)
{
    int[] outArr = new int[arr.Length];  // создаем новый массив
    for(int i =0;i<arr.Length;i++)      // проходим по всем элементам массива
    {
        outArr[i] = arr[arr.Length-1-i];  // в первую ячейку массива кладем элемент с конца массива
    }
    return outArr;      // возвращаем новый массив
}
