// Задайте двумерный массив из целых чисел. Напишите программу, которая удалит строчку и столбец,
...
// метод ищет наименьший элемент в двумерном массиве
...
int[,] Converted2DArray(int[,] arr, int x, int y)
{
    ...
    for (int i = 0; i < arr.GetLength(0); i++)
    {
        ...
        {
            if (i != x && j != y) // если i не равно х, j не равно у
            {
                outArr[k, m] = arr[i, j];
            }
            if (j != y) m++;
